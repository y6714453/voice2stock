...
def upload_to_yemot(wav_path):
    if not os.path.exists(wav_path):
        print("⚠️ קובץ WAV לא קיים – לא ניתן להעלות")
        return
        ...
        if file_path:
            ...
            if text and len(text.strip()) > 1:
                ...
                if symbol:
                    ...
                    if stock_text and len(stock_text.strip()) > 5:
                        print("📝 נוסח קולי:", stock_text)
                        await generate_edge_tts(stock_text)
                        if os.path.exists("output.wav"):
                            upload_to_yemot("output.wav")
                        else:
                            print("⚠️ קובץ output.wav לא נוצר – ייתכן שהייתה שגיאה ב־edge-tts או ffmpeg")
                    else:
                        print("⚠️ נוסח קולי ריק – דילוג")
                else:
